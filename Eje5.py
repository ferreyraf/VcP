...
def Func_Euclidia(tras_X, tras_Y, rot, img2,i_x, f_x, i_y, f_y):
    img2[:] = 0 
    M=np.array([[math.cos(rot),     math.sin(rot),  tras_X],
                [math.sin(-rot),        math.cos(rot),  tras_Y],
                [       0       ,       0       ,   1    ]])
...
while(1):
    ...
    if k == ord('e'):
        ...
        if i_y>f_y:
            ...
            f_y = aux
        img2=img.copy()
        Func_Euclidia(tras_X, tras_Y, rot, img2,i_x, f_x, i_y, f_y)
